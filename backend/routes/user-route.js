...
route
  ...
  .post('/login', async (req, res) => {
    ...
    if (user && (await bcrypt.compare(req.body.password, user.password))) {
      ...
      res.json({
        message: 'Something went wrong. Check pleasewhether your username or password is correct.',
      });
    }
  })
  .post('/logout', (req, res) => {
...
// delete user
  .delete('/', async (req, res) => {
    ...
    const adminStatus = req.session.user.status;
    if (
      (adminStatus === ('teacher' || 'chieftain') && userToDelete.status === 'student')
